...
async def fetch_pingzhong(client, code):
    ...
    return {"nav_date": date, "unit_nav": nav, "source": "eastmoney:pingzhongdata"}
...
async def fetch_fundgz(client, code):
    ...
    if nav is None or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", date):
        return None
    return {"nav_date": date, "unit_nav": nav, "source": "eastmoney:fundgz-dwjz"}
...
async def fetch_one(client, sem, item):
    async with sem:
        a, b = await asyncio.gather(
            fetch_pingzhong(client, item["code"]),
            fetch_fundgz(client, item["code"]),
...
async def main():
    funds = json.loads(FUNDS_FILE.read_text(encoding="utf-8"))["funds"]
    ...
    async with httpx.AsyncClient(follow_redirects=True, trust_env=False, limits=limits) as client:
        ...
        rows = await asyncio.gather(*(fetch_one(client, sem, x) for x in funds))
    ...
    with latest_csv.open("w", newline="", encoding="utf-8-sig") as f:
        fields = ["code", "name", "nav_date", "unit_nav", "source", "source_conflict", "error"]
